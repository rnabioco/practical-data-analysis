...
2. It's easier to adapt your code for different requirements since you only have to modify one place
3. They reduce the chance of mistakes from copying and pasting code
```{r}
...
out <- map(means, function(x) rnorm(10, x))
out

out <- map(means, ~ rnorm(10, .x))
out
...
### What are factors?
Factors are useful when trying to display categorical data in a non-alphabetical order. To manipulate factors tidyverse provides the forcats package.
```{r}
...
brauer_gene_exp %>%
  ...
  geom_point() +
  geom_smooth(method = "lm", se = F, size = 0.5) +
  facet_wrap(~ fct_relevel(nutrient, nutrient_levels)) +
  theme_cowplot() +
  theme(legend.position = "none")
...
brauer_gene_exp %>%
  ...
  geom_point() +
  geom_smooth(method = "lm", se = F, size = 0.5) +
  facet_wrap(~ fct_reorder(nutrient, expression, max, .desc = T)) +
  theme_cowplot() +
  theme(legend.position = "none")
...
#   geom_point() +
#   geom_smooth(method = "lm", se = F, size = 0.5) +
#   facet_wrap(~ fct_reorder(nutrient, slope, min)) +
#   theme_cowplot() +
#   theme(legend.position = "none")
```
