---
title: "Class 4: Genomic Interval Analysis Vignette"
...
We can now use the BED format as a common file to investigate multiple genome-wide datatypes. For example, let's examine a dataset of [`SNPs`](https://en.wikipedia.org/wiki/Single-nucleotide_polymorphism) taken from the ucsc genome browser. How would we go about finding `SNPs` that reside in known TF binding sites? 
...
snps <- read_tsv(snp_file, col_names = c("chrom",
                                 "start",
                                 "end",
                                 "name",
                                 "score",
...
tf_bed %>% 
  dplyr::filter(chrom == "chr22") -> chr22_tf_bed

# How to find intersecting intervals?
```
...
## Command-line tools
  ...
  - [`BedOPS`](https://bedops.readthedocs.io/en/latest/) (similar to BEDTools but offers unique functionality)
...
[`valr`](https://rnabioco.github.io/valr/) was developed to simplify genomic interval analysis and replace the need for learning the command line to perform interval analysis. We will use `valr` today to introduce genomic interval analysis. 
...
One of the most commone operations in interval analysis is to determine if intervals in 1 BED file overlap with intervals in another file. This operation can be used to answer many questions including:

1) Does a SNP overlap with a protein-coding region?
2) Which genes does a transcription factor bind to?
3) Which regions of the genome are depleted of genes?

The [`bed_intersect()`](https://rnabioco.github.io/valr/reference/bed_intersect.html) in valr function finds intervals that overlap in two datasets and reports the intersecting intervals.
...
We can find intersecting intervals with bed_intersect()
...
First read in a BED file that contains genes (on `chr22` for simplicity) in the human genome (hg19 build).
...
Next we'll generate a set of transcription start sites for each gene. The start site depends on the strand of the interval. Here we use `ifelse()` to conditionally define the start and end based on strand. 
...
1) The Encode project has performed `eCLIP` experiments to map RNA binding protein  (RBP) binding sites for > 100 RBPs. Go to the [Encode website](https://www.encodeproject.org/matrix/?type=Experiment), search for `eCLIP` experiments, select an RBP eClip experiment, and load a `BED` file (bed narrowPeak format) with the binding sites into R using just the url. Use the [`read_narrowPeak()`](https://rnabioco.github.io/valr/reference/read_bed.html) function from `valr`. Make sure that the file was generated using the `hg19` genome build. 

2) Use `bed_intersect()` to determine how many `eCLIP` peaks are found in genes and how many are intergenic (see the documention on the `invert` argument for `bed_intersect()`). 

3) Parse the output from question 2 to determine which gene has the most binding sites for your `RBP`. 

4) Use another tool from `valr` (overview and documentation [here](https://rnabioco.github.io/valr/articles/valr.html) to study your eClip binding sites and the `gene` bed files. Explain your question and answer.

# Quiz

Create an RMarkdown document and answer the questions from the Exercises section. Write your answers in the text, and print the final tibbles that your code produced that gave you the answer. Submit your final document as "Quiz 2" by Thurs at 10 PM.
