...
By default hierarchical clustering is performed using euclidian distance as a distance metric. There are many distance metrics, and some are more appropriate for different data. For this data, using euclidean distance will group the highly expressed genes (`h1` and `h2`) separate from the lowly expressed (`l1`, `l2`).  However we can see that the apttern of change over time is more similar between `h1` and `l1`

We can see the euclidian distances between genes using `dist`. 
...
We can change the distance metric using `clustering_distance_rows`. A correlation measure, is not sensitive to abundance, and can recover the shared patterns in the data.
...
How do we select out indidudal clusters? How do we define clusters?
