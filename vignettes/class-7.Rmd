...
By default hierarchical clustering is performed using euclidian distance as a distance metric. There are many distance metrics, and some are more appropriate for different data. For this data, using euclidean distance will group the highly expressed genes (`h1` and `h2`) separate from the lowly expressed (`l1`, `l2`).  However we can see that the pattern of change over time is more similar between `h1` and `l1`
