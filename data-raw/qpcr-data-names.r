...
sample_data <- crossing(sample, time, gene, rep, rt) %>%
  rowwise() %>%
  mutate(exp = ifelse(sample == "wt", sample(15:20, 1), sample(22:26, 1)))

plus_rts <- filter(sample_data, rt == "+")
minus_rts <- filter(sample_data, rt == "-") %>% mutate(exp = 0)

t0 <- filter(sample_data, time == 0) %>% rowwise() %>% mutate(exp = sample(5:15, 1))
t12 <- filter(sample_data, time == 12) %>% rowwise() %>% mutate(exp = sample(45:55, 1))
t24 <- filter(sample_data, time == 24) %>% rowwise() %>% mutate(exp = sample(95:100, 1))
t48 <- filter(sample_data, time == 48) %>% rowwise() %>% mutate(exp = sample(350:600, 1))
